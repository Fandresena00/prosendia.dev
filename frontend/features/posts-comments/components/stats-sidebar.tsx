...
export function StatsSidebar({ posts, commentStats }: StatsSidebarProps) {
  ...
  const replyRate =
    commentStats.total > 0
      ? Math.round(
          ((commentStats.repliedByAi + commentStats.repliedByHuman) /
            commentStats.total) *
            100,
        )
      : 0;
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full text-center px-6 gap-3 py-12">
        ...
        <p className="text-xs text-muted-foreground/60">
          Ajoutez un post pour voir les statistiques.
        </p>
      </div>
    );
  }
    ...
    <div className="p-5 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        {kpis.map((k) => (
          <div
            key={k.label}
            className={`rounded-xl border px-3 py-2.5 ${k.bg}`}
          >
      ...
      {commentStats.unanswered > 0 && (
        <div className="rounded-xl border border-rose-500/25 bg-rose-500/5 px-3.5 py-2.5">
          <p className="text-xs font-semibold text-rose-600">
            {commentStats.unanswered} commentaire
            {commentStats.unanswered > 1 ? "s" : ""} sans réponse
          </p>
          <p className="text-[10px] text-muted-foreground mt-0.5">
            Activez l&apos;IA sur les posts concernés pour les traiter
            automatiquement.
          </p>
        </div>
      )}
          ...
          <CardContent className="px-4 pb-4">
            <div className="flex items-center justify-center gap-4">
              <div className="relative">
                <ChartContainer config={BAR_CHART_CONFIG} className="h-25 w-25">
                  <PieChart>
                    <Pie
                      data={pieData}
                      cx="50%"
      ...
      {barData.length > 0 && (
        <Card className="border-border/40 bg-card/60 backdrop-blur-sm">
          ...
          <CardContent className="px-3 pb-4">
            <ChartContainer config={BAR_CHART_CONFIG} className="h-30 w-full">
              <BarChart
                data={barData}
                layout="vertical"
