...
export function usePostsComments() {
  ...
  const loadPosts = useCallback(async () => {
    ...
    setPostsError(null);
    try {
      const result = await api.getPosts(businessProfileId, 1, 50, postSearch || undefined);
  ...
  const { connected: realtimeConnected } = usePostsRealtime(
    businessProfileId,
    {
      /**
       ...
       */
      onPostUpdated: useCallback((postId: string, updates: Record<string, unknown>) => {
        setPosts((prev) =>
  ...
  const triggerAiReply = useCallback(async (commentId: string) => {
    ...
    setComments((prev) =>
      prev.map((c) =>
        c.id === commentId
          ? { ...c, isReplied: true, repliedByAi: true }
          : c,
      ),
    );
  }, []);
  ...
  const saveConfig = useCallback(async (form: Partial<PostAiConfigForm>) => {
    ...
    try {
      ...
      setConfigError(err instanceof Error ? err.message : "Erreur lors de la sauvegarde.");
      throw err;
    } finally {
  ...
  return {
    ...
    activePage, activePageKey, switchPage,
    ...
    postSearch, setPostSearch, loadPosts,

    // Auto-reply limit
    autoReplyCount, autoReplyLimitReached,
    ...
    commentSearch, setCommentSearch,
    commentPage, commentTotal,
    loadComments, syncComments, isSyncingComments,

    // Reply
    replyingTo, setReplyingTo,
    replyText, setReplyText,
    replySending, replyMode, setReplyMode,
    submitReply, triggerAiReply,

    // Config
    postAiConfig, configLoading, configSaving, configError, saveConfig,

    // Add / delete
    addDialogOpen, setAddDialogOpen,
    feedPosts, feedLoading, addingPost, deletingPostId,
    openAddDialog, addPost, deletePost,
  };
}
