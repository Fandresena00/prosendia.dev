...
export function AttachmentPreview({
  photos, file, preset,
  onRemovePhoto, onRemoveFile, onRemovePreset,
    ...
    <div className="px-3 pt-3 pb-1 flex flex-wrap gap-2 border-t border-border/40">
      {photos.map((p, i) => (
        <div key={i} className="relative group">
          <div className={`h-16 w-16 rounded-xl overflow-hidden bg-linear-to-br ${p.gradient}`}>
            {p.objectUrl ? (
              // Use plain <img> for blob: URLs — next/image requires width/height for non-fill
      ...
      {preset && (
        <div className="relative group">
          <div className="flex items-center gap-2 h-16 px-3 rounded-xl border border-primary/25 bg-primary/5">
            {/* Thumbnail of first preset photo */}
            <div className={`h-10 w-10 rounded-lg overflow-hidden bg-linear-to-br ${preset.photos[0]?.gradient ?? ""} flex items-center justify-center shrink-0`}>
              {preset.photos[0]?.objectUrl ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={preset.photos[0].objectUrl} alt={preset.name} className="w-full h-full object-cover" />
              ) : (
            ...
            <div className="min-w-0 max-w-[110px]">
              ...
              <p className="text-[10px] text-primary/70 truncate">
                {preset.photos.length > 1 ? `${preset.photos.length} photos` : "Photo prédéfinie"}
              </p>
            </div>
          </div>
