 ...
 *
 * CHANGE (realtime upgrade — inbox "AI is typing…" bubble):
 *   - executeReply() now wraps everything from the point userId is known in
 *     a try/finally that calls sseEmitter.aiTypingStop() no matter how the
 *     method exits (success send, escalation, fallback-no-credits, no
 *     messages, all models exhausted, or an unexpected throw). This is what
 *     the frontend's AiTypingBubble listens for to disappear immediately
 *     instead of relying on its 20s safety timeout.
 *   - aiTypingStart() is re-emitted right before the model call loop, in
 *     addition to the one InboxSyncSchedulerService/InboxSyncService already
 *     fire when the job is enqueued — if the job sat in the pg-boss queue
 *     long enough for the frontend's bubble to have already timed out, this
 *     restarts it right when generation actually begins.
 */
...
export class ReplyAiService {
  ...
  private async executeReply(
    ...
    // Everything below this point can flip the "AI is typing…" bubble on for
    // the agent — guarantee it always gets switched back off, on every exit
    // path (early return, escalation, success, or an unexpected throw).
    try {
      ...
      // Re-affirm the typing bubble right as generation actually starts —
      // if the job queued for a while, the frontend's own bubble may have
      // already hit its 20s safety timeout by now.
      ...
      // Whatever happened above — success, escalation, fallback, or a throw
      // that will bubble up to replyToConversation()'s catch — the agent's
      // "VendeoAI écrit…" bubble must close now.
      this.sseEmitter.aiTypingStop(userId, { conversationId });
    }
  }
