...
export class NotificationService {
  ...
  constructor(
    ...
    const vapidEmail = config.get<string>(
      'vapidEmail',
      'mailto:contact@vendeoai.com',
    );
  ...
  async notifyFacebookTokenExpired(
    userId: string,
    pageName: string,
  ): Promise<void> {
    await this.create({
  ...
  async checkUnansweredAndExpired(): Promise<void> {
    ...
    for (const conv of unansweredHuman) {
      ...
      const hoursWaiting = Math.floor(
        (Date.now() - lastMsg.createdAt.getTime()) / 3_600_000,
      );
  ...
  private async create(data: {
    ...
    if (
      shouldPush &&
      ['CRITICAL', 'WARNING', 'SUCCESS'].includes(data.severity)
    ) {
  ...
  private toDto(n: any): NotificationDto {
    return {
      ...
      conversationUrl: n.conversationId
        ? `/inbox?conv=${n.conversationId}`
        : null,
      facebookProfileUrl: n.clientFbPsid
      ...
      actionUrl:
        n.actionUrl ??
        (n.conversationId ? `/inbox?conv=${n.conversationId}` : null),
    };
  }
}
